...
export default {
  ...
  methods: {
    ...
    handle: throttle(async function (type, disNum, cart) {
      //type：为了区分这三个元素
      //disNum：add为1，minus为-1，change为当前文本框的值要整理
      //cart：获取产品的id
      switch (type) {
        case "add":
          disNum = 1;
          break;
        case "minus":
          //只要产品数量大于1才传递-1，当数量<=1则为0（表示原封不动）
      ...
      //派发actions
      await reqAddUpdateShopCart(cart.skuId, disNum);
      this.getData();
    }, 500),
    //删除某一个产品
